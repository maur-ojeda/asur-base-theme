...
function get_reusable_cpt_block_presense($post_id, $cpt_name, $cf_association, $block_index = 0)



{
    $selected = carbon_get_post_meta($post_id, $cf_association);
    if (empty($selected) || !isset($selected[$block_index]['id'])) {
        return null;
    }

    $block_id = (int) $selected[$block_index]['id'];
