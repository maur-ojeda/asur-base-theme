...
if ($footer_settings_query->have_posts()) {
    while ($footer_settings_query->have_posts()) {
        ...
        $footer_social_links = carbon_get_the_post_meta('footer_social_links');
    }
            ...
            <div class="container footer-content">
                    ...
                    <div class="col-12 col-lg-5 mb-4 mb-lg-0">
                        ...
                        <div class="d-flex gap-2">
                            <? foreach ($footer_social_links as $social_link) : ?>
                                <a class="text-white" href="<?php echo esc_url($social_link['social_url']); ?>" target="new">
                                    <i data-lucide="<?php echo esc_html($social_link['social_icon']); ?>"></i>
                ...
                <div class="row mt-5">
                    <div class="col-12 text-center">
                        <p class="small mb-0">Krom © <?php echo date('Y'); ?></p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-3 mb-3">
                ...
                <h5 class="fw-bold mt-3">Redes sociales</h5>
                <ul class="list-unstyled small">
                    <? foreach ($footer_social_links as $social_link) : ?>
                        <li><a class="text-white" href="<?php echo esc_url($social_link['social_url']); ?>" target="new">
                                <i data-lucide="<?php echo esc_html($social_link['social_icon']); ?>"></i></li>
        ...
        <div class="row">
            <div class="col-12 text-center border-top border-secondary pt-3 mt-3">
                <p class="small mb-0">Copyright © <?php echo date('Y'); ?></p>
            </div>
        </div>
    </div>
</footer>
...
<script>
    // Asume jQuery está presente (tu tema ya usa jQuery)
    jQuery(function($) {
        // Cuando el usuario cambie el select personalizado
        $(document).on('change', '.gt_selector', function() {
            var val = $(this).val();
            if (!val) return;
            // val viene en formato "es|en"
            var parts = val.split('|');
            var langCode = parts.length === 2 ? parts[1] : parts[0];

            // Buscar el select creado por Google Translate
            var $goog = $('.goog-te-combo');

            // Si el combo de Google existe - sincronizar y disparar change
            if ($goog.length) {
                $goog.val(langCode);
                $goog.trigger('change');
            } else {
                // Si no existe aún, intentar usar el API de GTranslate (fallback)
                // Muchos setups de GTranslate exponen `doGTranslate(langPair)` -> ej: 'es|en'
        ...
        // Opcional: si el combo de Google cambia (por ejemplo por widget), actualizar tu selector
        $(document).on('change', '.goog-te-combo', function() {
            var gval = $(this).val(); // ej "en"
            // mapear al formato "es|en" buscando una opción con *|en
            var $sel = $('.gt_selector');
            if (!$sel.length) return;
            var matched = $sel.find('option').filter(function() {
                return $(this).val().endsWith('|' + gval);
            }).first();
            if (matched.length) $sel.val(matched.val());
        });
    });
</script>
